...
function carregarTexto(categoria){
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'obter_texto_por_categoria', args: filtro}, function (resposta){
...
function carregaViradas() {
	ajustaTelaTexto();
	carregarTexto('viradas');
	
	selecionaAnoVirada();
}

function selecionaAnoVirada(ano){
	
	var listaViradas = '';
	
	if(ano != null){
		var filtro = JSON.stringify({ano: ano});
		$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'listar_viradas_por_ano', args: filtro}, function(resposta){
			var viradas = JSON.parse(resposta);
			if(viradas != null){
				$.each(viradas, function(i, virada){
					listaViradas += '<li class="lista-item"><a href="#" onclick="selecionaVirada('+virada.id+')">'+virada.nome+'</a></li>';
				});	
				montaListaViradas(ano, listaViradas);
			}
		});
	}
	else{
		montaListaViradas(ano, '');
	}
}

function montaListaViradas(ano, listaViradas){
	
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'listar_anos_de_viradas'}, function(resposta){
		var listaAnos = JSON.parse(resposta);
		
		var listaStr = '<ul>';
		listaStr += '	<li class="lista-titulo"><a>Viradas</a></li>';
		$.each(listaAnos, function(i, item){
			if(item.ano != '-'){
				listaStr += '<li class="lista-subtitulo" onclick="selecionaAnoVirada('+item.ano+')">'+item.ano+'</a></li>';
			}
			else{
				listaStr += '<li class="lista-subtitulo" onclick="selecionaAnoVirada(\''+item.ano+'\')">Desconhecido</a></li>';
			}
			if(ano == item.ano) listaStr += listaViradas;
		});
...
function selecionaVirada(idVirada){	
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'obter_virada_por_id', args: filtro}, function (resposta){
...
function carregaRitmos() {
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'listar_ritmos'}, function (resposta){
...
function selecionaRitmo(idRitmo){	
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'obter_ritmo_por_id', args: filtro}, function (resposta){
...
function carregaMusicas() {
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'listar_musicas'}, function (resposta){
...
function selecionaMusica(idMusica){	
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'obter_musica_por_id', args: filtro}, function (resposta){
...
function selecionaInstrumentoElemento(categoria){
	...
	if(categoria != null){
		...
		$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'listar_elementos_por_instrumento', args: filtro}, function(resposta){
...
function selecionaElemento(idElemento){	
	...
	$.post('/amigosratinhos/php/amigosratinhosDados.php', {funcao: 'obter_elemento_por_id', args: filtro}, function (resposta){
